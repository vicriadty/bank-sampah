                            ...
                            <tr>
                                ...
                                <td>
                                    @foreach ($setoran->details as $detail)
                                        {{ $detail->sampah->nama_jenis ?? '-' }}@if (!$loop->last)
                                            ,
                                        @endif
                                    @endforeach
                                </td>
...
@section('scripts')
    <script>
        ...
        var konversiCategories = konversiData.map(function(i) {
            return i.bulan;
        });
        var rupiahData = konversiData.map(function(i) {
            return parseFloat(i.total_rupiah);
        });
        var gramData = konversiData.map(function(i) {
            ...
            tooltip: {
                ...
                y: {
                    formatter: function(v, opts) {
                        return opts.seriesIndex === 0 ? 'Rp ' + v.toLocaleString('id-ID') : v.toFixed(4) + ' g';
                    }
                }
            },
            legend: {
